...
class InvoiceReportService(models.TransientModel): 
    ...
    def print_report(self):
        ...
        row += 1
        lines = 1
        for rowdata in self.env['account.move'].browse(self._context.get('active_ids')):
            col = 0
    ...
    def print_text_report(self):
        try:
            ...
            year = datetime.datetime.strptime(invoice_ids[0].invoice_date, '%Y-%m-%d').strftime('%Y')
            month = datetime.datetime.strptime(invoice_ids[0].invoice_date, '%Y-%m-%d').strftime('%m')
            ...
            for rowdata in invoice_ids:
                ...
                inv_val_6 = datetime.datetime.strptime(rowdata.invoice_date, '%Y-%m-%d').strftime('%Y%m%d')

                #7
                pay_date = ""
                if rowdata.pay_year and rowdata.pay_date: #pay_date ya no existe, ver cual lo sustituye
